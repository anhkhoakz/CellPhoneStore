...
const SearchPage = () => {
	...
	useEffect(() => {
		...
		let filteredResults = products.filter((item) => item.name.toLowerCase().includes(query?.toLowerCase()));
	...
	return (
		<Box sx={{ minHeight: "90vh", padding: 3, display: "flex", flexDirection: "column" }}>
			...
			{/* Phần sắp xếp */}
			<Box sx={{ mb: 3, display: "flex", justifyContent: "left", gap: 2 }}>
			...
			{results.length > 0 ? (
				<Grid container spacing={2} sx={{ flex: "1 1 auto", maxWidth: "80%", marginTop: "10px" }}>
					{results.map((product) => (
