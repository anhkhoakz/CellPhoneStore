...
const initialRows = [
    ...
    createData('FLAT50', '$50 off on orders over $200', '$50', 'Order > $200', '2024-11-30', 50, 10),
    createData('WELCOME15', '15% off on first purchase', '15%', 'First Purchase Only', '2025-01-01', 200, 30),
];
...
export default function DiscountsTable() {
    ...
    const handleDelete = () => {
        setRows(rows.filter(row => row.discountCode !== discountToDelete.discountCode));
    ...
    const handleEditDiscount = (updatedDiscount) => {
        setRows((prevRows) =>
            prevRows.map(row => (row.discountCode === updatedDiscount.discountCode ? updatedDiscount : row))
        );
                    ...
                    <TableBody>
                        {filteredRows
                            ...
                            .map((row, index) => {
                                ...
                                return (
                                    <StyledTableRow hover role="checkbox" tabIndex={-1} key={row.discountCode} isEven={isEven}>
                                        {columns.map((column) => {
