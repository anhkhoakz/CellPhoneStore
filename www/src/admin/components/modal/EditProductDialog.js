...
export default function EditProductDialog({ open, onClose, onSave, productData }) {
	...
	const handleRemoveColor = (index) => {
		const updatedvariants = editedProductData.variants.filter((_, i) => i !== index);
		setEditedProductData((prevData) => ({
			...prevData,
			variants: updatedvariants,
		}));
	...
	const isFormValid = () => {
		...
		return (
			...
			price &&
			stock &&
			variants.every((colorData) => colorData.name && colorData.image && colorData.price)
		);
	};
		...
		<Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
			...
			<DialogContent>
				...
				<div style={{ marginTop: "1.25em" }}>
					...
					<Button variant="outlined" color="primary" startIcon={<AddCircle />} onClick={handleAddColor}>
						Add Color
					</Button>
				</div>
			</DialogContent>
