...
import {
    ...
    TablePagination, TableRow, IconButton, FormControl,
    Select, MenuItem
} from '@mui/material';
...
const createData = (userId, name, phoneNumber, location, note, accountCreationDate, userStatus) => {
    return { userId, name, phoneNumber, location, note, accountCreationDate, userStatus };
};

const initialRows = [
    createData('USER123', 'John Doe', '123-456-7890', 'New York', '', '2024-01-15', 'Hoạt động'),
    createData('USER124', 'Jane Smith', '098-765-4321', 'California', 'Khóa tài khoản do hoạt động đáng ngờ', '2023-11-10', 'Đã khóa'),
    createData('USER125', 'Alice Johnson', '555-123-4567', 'Texas', '', '2024-06-25', 'Hoạt động'),
    createData('USER126', 'Bob Brown', '777-888-9999', 'Florida', '', '2023-08-13', 'Hoạt động'),
    createData('USER127', 'Charlie White', '222-333-4444', 'Nevada', 'Khóa tài khoản do lỗi thanh toán', '2024-02-20', 'Đã khóa'),
];
...
export default function UsersTable() {
    ...
    const handleSaveEditedUser = (updatedUser) => {
        setRows((prevRows) =>
            prevRows.map((row) => (row.userId === updatedUser.userId ? updatedUser : row))
        );
                                            ...
                                            return column.id === 'changeUserStatus' ? (
                                                <TableCell key={column.id} align="center" style={{ padding: '16px' }}>
                                                    <FormControl fullWidth>
                                                        ...
                                                        >
                                                            <MenuItem value="Hoạt động" sx={{ color: 'green', marginBottom: '4px' }}>Hoạt động</MenuItem>
                                                            <MenuItem value="Đã khóa" sx={{ color: 'red' }}>Đã khóa</MenuItem>
                                                        </Select>
                                                    </FormControl>
                                                </TableCell>
                                            ) : column.id === 'edit' ? (
